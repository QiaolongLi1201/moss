...
本文档定义 `server/agent/` 和 `server/dmoss/` 中每个文件的归属（D-Moss 框架 vs 产品宿主），
...
### 待完成
- 🔄 `provider-setup.ts`、`provider-defaults.ts`、`model-registry.ts` 等宿主侧配置管理尚在 `server/agent/`

## 状态说明

- ✅ **已迁移** — 代码已在 `packages/dmoss-agent/src/`，server/ 为 re-export
- 🔄 **待迁移** — 应属于 D-Moss，但尚未物理搬迁
- 🏠 **Host** — 属于产品宿主，不随 D-Moss 发布
...
| `agent.ts` | 🏠 | — | Host Agent 类（保留在宿主，组装产品特有功能） |
...
| `message-convert.ts` | 🏠 | — | Host 侧消息转换（保留） |
...
| `provider-setup.ts` | 🏠 | — | Host Provider 配置（保留在宿主） |
| `provider-defaults.ts` | 🏠 | — | Host 默认配置加载（保留在宿主） |
| `model-registry.ts` | 🏠 | — | Host 模型能力注册（保留在宿主） |
| `spawn-profile.ts` | 🏠 | — | Host 子代理配置（保留在宿主） |
...
## `server/agent/` → Host（不随 D-Moss 发布）

| 文件 | 说明 |
|------|------|
| `cli.ts` | Host CLI 入口（绑定产品特有命令） |
| `sdk.ts` | Host SDK 入口 |
| `channels/weixin.ts` | 微信通道适配 |
| `channels/feishu.ts` | 飞书通道适配 |
| `gateway/` | 网关相关 |
| `openclaw-index.ts` | OpenClaw 模块桶文件 |
| `dmoss-tool-hooks.ts` | Host 特有工具钩子 |
| `tools/device-tools.ts` | Host 设备工具实现 |
| `tools/host-tools.ts` | Host 专有工具 |
| `tools/attachment-tools.ts` | Host 附件工具 |
...
| `delegation.ts` | 🏠 | 委派路径决策（Host 特有） |
| `permission-guard.ts` | 🏠 | 权限守卫（Host 特有） |
...
| `system-prompt-layers.ts` | 🏠 | 提示分层（Host 特有） |
| `types.ts` | 🏠 | 请求/附件/策略类型（Host 特有） |

## `server/dmoss/` → Host

| 文件 | 说明 |
|------|------|
| `app.ts` | Host 编排器（消费 DmossAgent） |
| `open-web-intent.ts` | 打开网页意图与宿主打开 URL 工具提示路由（宿主能力，非 `@rdk-moss/agent`） |
| `system-prompt-builder.ts` | Host 专有提示内容 |
| `tool-contract-prompt.ts` | Host 工具描述 |
...
| `index.ts` | 🏠 Host（编排入口，调用 bootstrap + robot-hub 加载） |
| `robot-hub-store.ts` | 🏠 Host（文件系统持久层，依赖 `resolveDataDir()`；类型已从 `@rdk-moss/agent/knowledge` 导入） |
| `robot-hub-types.ts` | ✅ 已迁移至 `@rdk-moss/agent/knowledge/robot-hub-types.ts`（Host 侧为 re-export） |
...
| `bootstrap.ts` | ✅ (框架在 agent，Host 注册工厂) |
| `extension-device-tools.ts` | 🏠 Host（依赖 Host Tool 类型与厂商插件注册表） |
| `builtins/product-platform-extension.ts` | 🏠 Host |

---

**统计**: ✅ 已迁移 ~42 | 🏠 Host 保留 ~33 | 🔄 待迁移 0

## 宿主集成 API

当 `@rdk-moss/agent` 作为独立开源包时，宿主通过以下 API 注入产品特有配置：
...
| `registerKnowledgeModule(mod)` | 注册领域知识模块 | Host knowledge bootstrap |
| `syncPlatformExtensionsAtStartup()` | 启动时同步平台扩展 | `bootstrap.ts` |

核心 Agent 运行时已基本完成迁移。剩余 🏠 标记文件属于产品宿主层，不随 `@rdk-moss/agent` 发布。

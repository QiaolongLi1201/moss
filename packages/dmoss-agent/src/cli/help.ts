...
export function displayHelp(c: Colors, options: { all?: boolean } = {}): void {
  ...
  if (!options.all) {
    const lines = [
      ...
      `    Priority: ${c.bold('CLI flags/-c')} > ${c.bold('environment variables')} > ${c.bold('project .moss/config.json')} > ${c.bold('user config')} > ${c.bold('built-in default')}.`,
      '',
  ...
  const lines = [
    ...
    '',
    `  ${c.bold('Environment')}`,
    `    ${c.magenta('DMOSS_PROVIDER')}          ${c.dim('deepseek | qwen | openai | anthropic | openai-compatible')}`,
    `    ${c.magenta('DMOSS_API_KEY')}           ${c.dim('generic key for your own provider; not needed for the built-in model gateway')}`,
    `    ${c.magenta('DEEPSEEK_API_KEY')}        ${c.dim('DeepSeek API key fallback')}`,
    ...
    `    ${c.magenta('DMOSS_MODEL')}             ${c.dim('model name override; built-in default uses Moss')}`,

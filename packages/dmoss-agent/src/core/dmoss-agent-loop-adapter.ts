...
export function createDmossAgentLoopEventAdapter(
  ...
  const getResult = (result: MiniAgentResult): ChatResult => ({
    ...
    ...(steeringEvents.length > 0 ? { steeringEvents } : {}),
    stopReason:
  ...
  return {
    onMiniEvent(event) {
      switch (event.type) {
        ...
        case 'turn_end':
          return [{ type: 'turn_end', turn: event.turn, stopReason }];
        ...
        case 'steering':
          steeringEvents.push(`pending:${event.pendingCount}`);
          return [{ type: 'steering', pendingCount: event.pendingCount, firedRules: [] }];
        case 'run_metrics':

...
export async function executeOneToolCall(
  ...
  const eligibleForRetry =
    TRANSIENT_RETRY_TOOLS.has(call.name) && !approvalTriggered;
  ...
  for (let attempt = 0; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (!skipAgentHeartbeat && attempt === 0) {
      ...
      attemptText = await Promise.race([
        abortable(
          tool.execute(call.input, callToolCtx),
      ...
      if (perToolAbortSignal?.aborted && !deps.abortSignal.aborted) {
        aborted = { by: 'user' };
        attemptText = 'Execution error: aborted_by_user: user cancelled this tool execution';

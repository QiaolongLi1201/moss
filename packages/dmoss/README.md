...
**API stability:** Anything exported from `src/index.ts` and the `package.json` `exports` map is treated as **stable** within a major version. Do not import deep paths that are not listed in `exports`. For the agent runtime, see [`@rdk-moss/agent` API stability](../dmoss-agent/API.md#api-stability-labels).
